<?php

/**
 * Lang.php
 *
 * @copyright  2026 beikeshop.com - All Rights Reserved
 * @link       https://beikeshop.com
 * @author     guangda <service@guangda.work>
 * @created    2023-09-09 09:09:09
 * @modified   2026-05-28 06:35:56
 */

return [
    'add_url'                => 'url 추가',
    'add_url_help'           => '유튜브 동영상과 mp4 형식의 원격 동영상 링크만 지원한다',
    'add_variable'           => '사양 추가',
    'add_variable_image'     => '사양 이미지 추가',
    'add_variable_value'     => '사양값 추가',
    'basic_information'      => '기초정보',
    'batch_active'           => '대량 로딩',
    'batch_delete'           => '대량 삭제',
    'batch_inactive'         => '대량 제거',
    'category_already'       => '클래스가 이미 존재함',
    'category_disabled'      => '분류가 비활성화되었습니다',
    'category_placeholder'   => '선택/검색하십시오',
    'clear_restore'          => '휴지통 비우기',
    'confirm_batch_product'  => '선택한 상품을 일괄 삭제하시겠습니까?',
    'confirm_batch_restore'  => '선택한 상품을 복구하시겠습니까?',
    'confirm_batch_status'   => '선택한 상품의 상태를 일괄 수정하시겠습니까?',
    'confirm_delete_restore' => '휴지대를 비우시겠습니까?',
    'cost_price'             => '원가',
    'default_main_product'   => '기본 기본 상품',
    'drag_drop_upload'       => '그림을 끌어 올립니다',
    'enable_multi_spec'      => '멀티 스펙 사용',
    'file_manager_select'    => '그림 갤러리에서 선택',
    'full_edit'              => '전체 화면 편집',
    'iframe_code'            => 'iframe 내장 코드',
    'iframe_code_hint'       => '동영상 사이트 아이프라메 (iframe)에는 임베디드 코드가 있다. 예를 들어 유튜브 동영상 아래 임베디드 버튼을 클릭하면 임베디드 안에 있는 코드를 복사한다',
    'image_help'             => '첫 번째 이미지는 상품 메인 이미지로 여러 장의 이미지를 동시에 업로드 할 수 있도록 지원되며, 여러 장의 이미지 간 위치 조정이 가능합니다',
    'images_video'           => '이미지/비디오',
    'model'                  => '모델',
    'modify_order'           => '수정, 드래그 조정 순서 더블 클릭',
    'origin_price'           => '원가',
    'price'                  => '가격',
    'product_details'        => '상품상세',
    'product_relations'      => '관련 상품',
    'products_create'        => '상품 만들기',
    'products_delete'        => '상품 삭제',
    'products_filter_index'  => '고급 필터 보기',
    'products_filter_update' => '고급 편집',
    'products_index'         => '상품 목록',
    'products_restore'       => '휴지통 복구',
    'products_show'          => '상품상세',
    'products_trashed'       => '휴지통',
    'products_update'        => '업데이트 상품',
    'products_upload_new'    => '새 그림 업로드',
    'quantity'               => '수량',
    'sku_error_repeat'       => 'sku 반복',
    'sku_hint'               => '참고:권장 다중 스펙 수는 100개를 초과하지 않으며, 페이지 카피, 렌더링 시간이 길어지고 전송 데이터가 너무 커서 잘릴 수 있습니다',
    'stocks'                 => '상품재고',
    'video_help'             => '힌트가 시스템 크기 제한을 초과하면 php.ini의 post_max_size를 수정한다',
    'video_local'            => '로컬',
    'video_path'             => '비디오 주소',
    'video_path_hint'        => '원격 비디오는.mp4로 끝나는 비디오 주소여야 한다',
    'view_more'              => '미리 보기',
    'weight_class'           => '무게 단위',
    'weight_text'            => '무게',
];
