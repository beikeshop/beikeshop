...
abstract class FileRepository implements RepositoryInterface, Countable
{
    ...
    public function scan()
    {
        ...
        foreach ($paths as $key => $path) {
            $manifests = $this->getFiles()->glob(storage_path('app/plugins/*.json'));
    ...
    {
        return $this->cache->store($this->config->get('plugins.cache.driver'))->remember($this->config('cache.key'), $this->config('cache.lifetime'), function () {
