...
class CheckLangCommand extends BaseCommand
{
     ...
     * @var string
     */
    protected $description = 'Check missing language keys in the specified plugin.';

    public function __construct()
    {
        parent::__construct();

        $this->langPath = DIRECTORY_SEPARATOR . config('plugins.paths.generator.lang.path', 'Resources/lang');
    }
    ...
    private function getDirectories($module)
    {
        ...
        if (count($directories) == 0) {
            $this->components->info("No language files found in plugin $moduleName");

            return false;
        }

        if (count($directories) == 1) {
            $this->components->warn("Only one language file found in plugin $moduleName");
