...
@if ($product['video'] && strpos($product['video'], '<iframe')===false) <script src="{{ asset('vendor/video/video.min.js') }}">
  </script>
      ...
      <div class="col-12 col-lg-6">
        <div class="peoduct-info product-mb-block">
          ...
          <div class="rating-wrap d-lg-flex">
            <div class="rating">
              @for ($i = 0; $i < 5; $i++) <i class="iconfont">&#xe628;</i>
                @endfor
            </div>
          ...
          <div class="variables-wrap mb-md-4" v-if="source.variables.length">
            <div class="variable-group" v-for="variable, variable_index in source.variables" :key="variable_index">
              ...
              <div class="variable-info">
                <div v-for="value, value_index in variable.values" @click="checkedVariableValue(variable_index, value_index, value)" :key="value_index" data-bs-toggle="tooltip" data-bs-placement="top" :title="value.image ? value.name : ''" :class="[value.selected ? 'selected' : '', value.disabled ? 'disabled' : '', value.image ? 'is-v-image' : '']">
          ...
          <div class="product-btns">
            ...
            <div class="quantity-btns">
              ...
              <button class="btn btn-outline-dark ms-md-3 add-cart fw-bold" :product-id="product.id" :product-price="product.price" :disabled="!product.quantity" @click="addCart(false, this)"><i class="bi bi-cart-fill me-1"></i>{{ __('shop/products.add_to_cart') }}
              </button>
              @endhookwrapper
              @hookwrapper('product.detail.buy_now')
              <button class="btn btn-dark ms-3 btn-buy-now fw-bold" :disabled="!product.quantity" :product-id="product.id" :product-price="product.price" @click="addCart(true, this)"><i class="bi bi-bag-fill me-1"></i>{{ __('shop/products.buy_now') }}
              </button>
              @endhookwrapper
          ...
          <div>
            ...
            <div style="display: flex;">
              <div style="display: flex; flex-direction:column; width:100px">
                <img style="width: 100px;" src="https://shadow.elemecdn.com/app/element/hamburger.9cf7b091-55e9-11e9-a976-7f4d0b07eef6.png" />
                <div>
                  <input id="c1" type="checkbox" :value="product.id">
                  <label for="c1">华为超级快充无线充电</label>
                </div>
              </div>
              <div style="display: flex; flex-direction:column; width:100px">
                <img style="width: 100px;" src="https://shadow.elemecdn.com/app/element/hamburger.9cf7b091-55e9-11e9-a976-7f4d0b07eef6.png" />
                ...
                <div>
                  <input id="c2" type="checkbox" value="1">
                  <label for="c2"> HUAWEI Tag 轻薄小巧</label>
                </div>
              </div>
            </div>
            <div>
              <!-- 加入购物车、立即购买 -->
              <button class="btn btn-outline-dark ms-md-3 add-cart fw-bold" :product-id="product.id" :product-price="product.price" :disabled="!product.quantity" @click="addCart(false, '324',1)"><i class="bi bi-cart-fill me-1"></i>{{ __('shop/products.add_to_cart') }}
              </button>
            </div>
          </div>
  ...
  <script>
    ...
    let app = new Vue({
      ...
      methods: {
        ...
        addCart(isBuyNow = false, sku_id = "", quantity = 1) {
          let options = {
          ...
          if (sku_id !== "") {
            ...
            console.debug("推荐商品加入购物车", sku_id, quantity)
          }
          bk.addCart(options, null, () => {
            if (isIframe) {
