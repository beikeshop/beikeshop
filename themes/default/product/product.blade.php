...
<div class="container {{ request('iframe') ? 'pt-4' : '' }}" id="product-app" v-cloak>
  <div class="row mb-md-5 mt-md-0" id="product-top">
    <div class="col-12 col-lg-6 mb-2">
      ...
      <div class="right" id="zoom">
        ...
        <div class="product-img"><img :src="images.length ? images[0].preview : '{{ asset('image/placeholder.png') }}'"
          class="img-fluid">
        </div>
      </div>
    @else
    ...
    <div class="swiper" id="swiper-mobile">
      <div class="swiper-wrapper">
      <div class="swiper-slide d-flex align-items-center justify-content-center" v-for="image, index in images"
        :key="index">
  ...
  @endif
    ...
    @endif
        ...
        <div class="variables-wrap mb-md-4" v-if="source.variables.length">
          <div class="variable-group" v-for="variable, variable_index in source.variables" :key="variable_index">
            ...
            <div class="variable-info">
              <div v-for="value, value_index in variable.values"
                @click="checkedVariableValue(variable_index, value_index, value)" :key="value_index"
                data-bs-toggle="tooltip" data-bs-placement="top" :title="value . image ? value . name : ''"
        ...
        <button class="btn btn-outline-dark ms-md-3 add-cart fw-bold" :product-id="product.id"
          :product-price="product.price" :disabled="!product . quantity" @click="addCart(false, this)"><i
          class="bi bi-cart-fill me-1"></i>{{ __('shop/products.add_to_cart') }}
        </button>
        @endhookwrapper
        @hookwrapper('product.detail.buy_now')
        <button class="btn btn-dark ms-3 btn-buy-now fw-bold" :disabled="!product . quantity"
          :product-id="product.id" :product-price="product.price" @click="addCart(true, this)"><i
          class="bi bi-bag-fill me-1"></i>{{ __('shop/products.buy_now') }}
        </button>
        @endhookwrapper
    ...
    @endif
      @else
      <div class="text-danger"><i class="bi bi-exclamation-circle-fill"></i> {{ __('product.has_been_inactive') }}
      </div>
    @endif
        </div>
        ...
        <div>
          ...
          <div>
            <!-- 加入购物车、立即购买 -->
            <button class="btn btn-outline-dark ms-md-3 add-cart fw-bold" :product-id="product.id"
              :product-price="product.price" :disabled="!product.quantity" @click="addCart1(product.id)"><i
                class="bi bi-cart-fill me-1"></i>{{ __('shop/products.add_to_cart') }}
            </button>
          </div>
        </div>
