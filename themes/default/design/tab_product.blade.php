    ...
    <div class="container">
      @if ($content['tabs'] ?? false)
        <div class="nav justify-content-center mb-3">
          ...
          <a class="nav-link {{ $content['editableTabsValue'] == $key ? 'active' : '' }}" href="#tab-product-{{ $loop->index }}" data-bs-toggle="tab">{{ $tabs['title'] }}</a>
          @endforeach
        </div>
        <div class="tab-content">
          @foreach ($content['tabs'] as $key => $products)
          <div class="tab-pane fade show {{ $content['editableTabsValue'] == $key ? 'active' : '' }}" id="tab-product-{{ $loop->index }}">
            <div class="row">
