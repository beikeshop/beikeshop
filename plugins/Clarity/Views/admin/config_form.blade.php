...
<div class="mb-5" id="app">
  <el-form ref="form" :model="form" class="form-wrap" :inline-message="true">
    <table class="table table-bordered">
      ...
      <tbody v-if="form.clarity.length">
      <tr v-for="(item, index) in form.clarity" :key="index">
        <td>
          <el-form-item prop="provider" class="mb-0">
            <el-select size="small" v-model="item.provider" @change="(e) => {providerChange(e, index)}"
                       placeholder="{{ __('Social::setting.provider') }}">
              <el-option
        ...
        <td>
          <el-form-item
            label="" :prop="`clarity[${index}].key`" class="mb-0"
            :rules="[
        ...
        <td>
          <el-form-item
            label="" :prop="`clarity[${index}].secret`" class="mb-0"
            :rules="[
          ...
          >
            <el-input size="small" v-model="item.secret"
                      placeholder="{{ __('Social::setting.entry_secret') }}"></el-input>
          </el-form-item>
        </td>
        <td>
          <el-form-item label="" class="mb-0">
            <div class="input-group">
              <input size="small" class="form-control" :value="item.callback"
                     placeholder="{{ __('Social::setting.entry_callback') }}"></input>
              <a href="javascript:void(0)" class="btn btn-outline-secondary opacity-75 copy-code"
                 :data-clipboard-text="item.callback" @click="copyCode"><i class="bi bi-front"></i></a>
            </div>
          </el-form-item>
        </td>
        <td>
          <el-form-item label="" prop="sort_order" class="mb-0">
            <el-input size="small" v-model="item.sort_order"
                      placeholder="{{ __('Social::setting.entry_sort_order') }}"></el-input>
          </el-form-item>
        </td>
        <td class="text-end">
          <button type="button" @click="form.clarity.splice(index, 1)" class="btn btn-outline-danger btn-sm ml-1"><i
...
<script>
  ...
  let app = new Vue({
    ...
    data: {
      form: {
        clarity: @json($plugin->getSetting('setting') ?? []),
      },
    ...
    computed: {
      providers() {
        ...
        providers.forEach(e => {
          if (this.form.clarity.some(s => s.provider == e.code)) {
            e.disabled = true;
          }
        })
    ...
    methods: {
      submit(form) {
        this.$refs[form].validate((valid) => {
          ...
          $http.post("{{ admin_route('plugin.clarity.setting') }}", this.form.clarity).then((res) => {
      ...
      providerChange(e, index) {
        this.form.clarity[index].callback = `{{ shop_route('home.index') }}/clarity/callbacks/${e}`
      },

      addRow() {
        let providers = this.source.providers.filter(e => !this.form.clarity.some(s => s.provider == e.code))
        if (providers.length) {
          this.form.clarity.push({
            ...
            key: '',
            secret: '',
            callback: `{{ shop_route('home.index') }}/clarity/callbacks/${this.source.providers[0].code}`,
            sort_order: this.form.clarity.length
          })
        }
      },
